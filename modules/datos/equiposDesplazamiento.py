...
class EquiposDesplazamiento:
    ...
    def validarMarcadoCheckbox(parent_item, column, obtenerEquiposMarcados):
        ...
        try:
            if codigo.isdigit():
                EquiposDesplazamiento.marcardesmarcar_todos_hijos(parent_item, estado)
                if codigo != "0": 
                    EquiposDesplazamiento.actualizar_estado_padre_hijos(parent_item)     
            else:  
                EquiposDesplazamiento.marcardesmarcar_todos_hijos(parent_item, estado)
                EquiposDesplazamiento.actualizar_estado_padre_hijos(parent_item)
        finally:
    ...
    def obtener_todos_elementos_marcados(tree_widget):
        ...
        for i in range(tree_widget.topLevelItemCount()):
            ...
            resultado_grupo = EquiposDesplazamiento.obtener_elementos_marcados_recursivo(grupo)
            if resultado_grupo:
    ...
    def obtener_elementos_marcados_recursivo(nodo):
        ...
        if nodo.childCount() > 0:
            ...
            for i in range(nodo.childCount()):
                hijo = nodo.child(i)
                resultado_hijo = EquiposDesplazamiento.obtener_elementos_marcados_recursivo(hijo)
                if resultado_hijo:
    ...
    def obtener_elementos_des_marcados_recursivo(nodo):
        if nodo.parent():
            return EquiposDesplazamiento.obtener_elementos_des_marcados_recursivo(nodo.parent())
        else:
            marcados = EquiposDesplazamiento.obtener_elementos_marcados_recursivo(nodo)
        return marcados
    ...
    def obtener_todos_elementos_arbol(tree_widget):
        ...
        for i in range(tree_widget.topLevelItemCount()):
            grupo = tree_widget.topLevelItem(i)
            resultado_grupo = EquiposDesplazamiento.obtener_elementos_arbol_recursivo(grupo)
            if resultado_grupo:
    ...
    def obtener_elementos_arbol_recursivo(nodo):
        ...
        if nodo.childCount() > 0:
            ...
            for i in range(nodo.childCount()):
                hijo = nodo.child(i)
                resultado_hijo = EquiposDesplazamiento.obtener_elementos_arbol_recursivo(hijo)
                if resultado_hijo:
                    if isinstance(resultado_hijo, dict):
                        marcados.update(resultado_hijo)
                    else:
    ...
    def abrirMenuGlobalProyecto(pos_global, treeWidget, callback_graficar):
        ...
        marcar_todo.triggered.connect(lambda: EquiposDesplazamiento.marcar_desmarcar_proyecto_completo(
            treeWidget, Qt.Checked, callback_graficar))
        
        desmarcar_todo.triggered.connect(lambda: EquiposDesplazamiento.marcar_desmarcar_proyecto_completo(
