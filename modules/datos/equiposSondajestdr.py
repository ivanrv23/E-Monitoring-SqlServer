...
class EquiposSondajestdr:
    ...
    def validarMarcadoCheckbox(parent_item, column, treeWidget, graficarEquiposMarcados):
        ...
        if estado != Qt.CheckState.PartiallyChecked:
            ...
            if codigo.isdigit():
                equipos = EquiposSondajestdr.obtener_todos_elementos_arbol(treeWidget)
                if codigo == "0": # COMPONENTE
                    if estado == Qt.Checked:
                        if EquiposSondajestdr.validarMarcadoUnEquipoZona(equipos, nombremarcado):
                            # limpiar inclinometros
                            EquiposSondajestdr.limpiarEquiposTipo(treeWidget)
                            parent_item.setCheckState(0, Qt.Checked)
                            EquiposSondajestdr.marcardesmarcar_todos_hijos(parent_item, estado)
                        else:
                            if estado == Qt.CheckState.Checked:
                                mostrar_mensaje("MARCACIÓN MÚLTIPLE", "Solo debe marcar un TDR.", "advertencia")
                                EquiposSondajestdr.actualizar_estado_padre_padre(parent_item)
                    elif estado == Qt.Unchecked:
                        EquiposSondajestdr.marcardesmarcar_todos_hijos(parent_item, estado)
                else:
                    zonamarcada = (parent_item.parent().text(0), parent_item.parent().text(2), parent_item.parent().text(3))
                    if EquiposSondajestdr.validarMarcadoUnEquipoZona(equipos, zonamarcada):
                        # limpiar sondajes
                        EquiposSondajestdr.limpiarEquiposTipo(treeWidget)
                        parent_item.setCheckState(0, estado)
                        EquiposSondajestdr.marcardesmarcar_todos_hijos(parent_item, estado)
                        EquiposSondajestdr.actualizar_estado_padre_hijos(parent_item)
                    else:
                        if estado == Qt.CheckState.Checked:
                            mostrar_mensaje("MARCACIÓN MÚLTIPLE", "Solo debe marcar un TDR.", "advertencia")
                            EquiposSondajestdr.actualizar_estado_padre_padre(parent_item)
    ...
    def validarOpcionesMenuCheckbox(point, treeWidget, graficarnuevafechasinclinometros, reiniciarvistas):
        ...
        if item:
            ...
            if tipo.isdigit():
                ...
                elif tipo == "1": # Sondajes tdr
                    idzona = item.text(2)
                    idproyecto = item.text(3)
                    edit_sondajes = menu.addAction("Cambiar de Componente")
                    delete_sondajes = menu.addAction("Eliminar TDRs")
                    edit_sondajes.triggered.connect(lambda: SubirTDR.cambiar_componente_sondajestdr(idzona, idproyecto, treeWidget, "TDR", "1", "sondajetdr", reiniciarvistas))
                    delete_sondajes.triggered.connect(lambda: SubirTDR.eliminar_sondajestdr(idproyecto, idzona, "TDR", "1", treeWidget, reiniciarvistas))
            else:    
                if tipo == "sondajetdr":
                    ...
                    if estado == Qt.Checked:
                        ...
                        if lista:
                            inclifechas = EquiposSondajestdr.obtenerListaFechasEquipoMarcado(lista, "TDR", idcomponente, idinstrumento)
                        else:
                            inclifechas = None
                    else:
                        inclifechas = None
                    date_sondaje = menu.addAction("Marcar/Desmarcar Fechas")
                    edit_sondaje = menu.addAction("Editar TDR")
                    delete_sondaje = menu.addAction("Eliminar TDR")
                    date_sondaje.triggered.connect(lambda: SubirTDR.mostrarDialogoFechasSondajestdr(treeWidget, inclifechas, idproyecto, idcomponente, idinstrumento, nombrezona, nombretdr, estado, graficarnuevafechasinclinometros))
                    edit_sondaje.triggered.connect(lambda: SubirTDR.actualizarSondajeTDR(idproyecto, idcomponente, idinstrumento, treeWidget, "TDR", "1", "sondajetdr", reiniciarvistas))
                    delete_sondaje.triggered.connect(lambda: SubirTDR.eliminar_sondajetdr(idproyecto, idinstrumento, nombretdr, "TDR", "sondajetdr", treeWidget, reiniciarvistas))
    ...
    def marcardesmarcar_todos_hijos(nodo, estado):
        for i in range(nodo.childCount()):
            ...
            EquiposSondajestdr.marcardesmarcar_todos_hijos(hijo, estado)
    ...
    def actualizar_estado_padre_hijos(nodo):
        ...
        if nodo.parent():
            if nodo.parent().parent():
                EquiposSondajestdr.actualizar_estado_padre_hijos(nodo.parent())
    ...
    def obtener_todos_elementos_marcados(tree_widget):
        ...
        for i in range(tree_widget.topLevelItemCount()):
            ...
            resultado_grupo = EquiposSondajestdr.obtener_elementos_marcados_recursivo(grupo)
            if resultado_grupo:
    ...
    def obtener_elementos_marcados_recursivo(nodo):
        ...
        if nodo.childCount() > 0:
            ...
            for i in range(nodo.childCount()):
                hijo = nodo.child(i)
                resultado_hijo = EquiposSondajestdr.obtener_elementos_marcados_recursivo(hijo)
                if resultado_hijo:
    ...
    def obtener_elementos_des_marcados_recursivo(nodo):
        if nodo.parent():
            return EquiposSondajestdr.obtener_elementos_des_marcados_recursivo(nodo.parent())
        else:
            marcados = EquiposSondajestdr.obtener_elementos_marcados_recursivo(nodo)
        return marcados
    ...
    def obtener_todos_elementos_arbol(tree_widget):
        ...
        for i in range(tree_widget.topLevelItemCount()):
            grupo = tree_widget.topLevelItem(i)
            resultado_grupo = EquiposSondajestdr.obtener_elementos_arbol_recursivo(grupo)
            if resultado_grupo:
    ...
    def obtener_elementos_arbol_recursivo(nodo):
        ...
        if nodo.childCount() > 0:
            ...
            for i in range(nodo.childCount()):
                hijo = nodo.child(i)
                resultado_hijo = EquiposSondajestdr.obtener_elementos_arbol_recursivo(hijo)
                if resultado_hijo:
                    if isinstance(resultado_hijo, dict):
                        marcados.update(resultado_hijo)
                    else:
    ...
    def obtenerListaFechasEquipoMarcado(lista, tipolista, idcomponente, idinstrumento):
        for region, instrumentos in lista.items():
            for tipo, listasondajes in instrumentos.items():
                if tipo[0] == tipolista and tipo[1] == idcomponente:
                    for nombreincli, idinstru, fechas in listasondajes:
        ...
        if proyecto_id:
            ...
            if componentes:
                for zona in componentes:
                    ...
                    if sondajestdr:
                        TreeCheckbox.crearNuevoGrupoCheckboxesSimple(tree_widget, namezona, idzona, proyecto_id, "TDR", "1", sondajestdr, "sondajetdr", "SI")
