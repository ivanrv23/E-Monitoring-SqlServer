...
def procesar_grafica(widget, labeltendencia, data, idx_nombre, idx_fecha, idx_lectura, labelejex, labelejey, tipo, medida, tiempo, titulo, idproyecto, modulo, pluviometro_data=None, equipostendencia=None, escala=None, fecha_inicio=None, fecha_fin=None):
    ...
    # --- Controles de paginación de leyenda (solo visibles si hay demasiadas series) ---
    widget.boton_anterior_leyenda = QPushButton("\u25c0")
    widget.boton_anterior_leyenda.setFixedWidth(28)
    widget.boton_anterior_leyenda.setStyleSheet("font-size: 11px; padding: 2px;")
    widget.lbl_pagina_leyenda = QLabel("")
    widget.lbl_pagina_leyenda.setStyleSheet("font-size: 11px; font-weight: bold; margin: 0 4px;")
    widget.boton_siguiente_leyenda = QPushButton("\u25b6")
    widget.boton_siguiente_leyenda.setFixedWidth(28)
    widget.boton_siguiente_leyenda.setStyleSheet("font-size: 11px; padding: 2px;")
    widget.boton_anterior_leyenda.setVisible(False)
    widget.lbl_pagina_leyenda.setVisible(False)
    widget.boton_siguiente_leyenda.setVisible(False)
    toolbar_layout.addWidget(widget.boton_anterior_leyenda)
    toolbar_layout.addWidget(widget.lbl_pagina_leyenda)
    toolbar_layout.addWidget(widget.boton_siguiente_leyenda)
    ...
    # --- Estado y límites de paginación de la leyenda ---
    estado_leyenda = {'pagina': 0, 'total_paginas': 1}
    FRACCION_MAX_ALTURA_LEYENDA = 0.42  # % del alto del canvas que la leyenda puede ocupar como máximo
    FILAS_LEYENDA_MIN = 2
    FILAS_LEYENDA_MAX = 9
    ...
    def calculate_columns():
        ...
        for label in leyenda_labels:
            ...
            bbox_texto = text_obj.get_window_extent(renderer)
            max_width = max(max_width, bbox_texto.width + (leyendazise*2.2))
            max_height = max(max_height, bbox_texto.height)
            text_obj.remove()

        # Ancho real del widget en píxeles de pantalla. OJO: al arrancar la
        # vista, Qt puede no haber terminado de acomodar el layout todavía y
        # widget.width() puede devolver un valor chico/desactualizado. Por eso
        # la primera llamada a actualizar_leyenda() se difiere con QTimer
        # (ver más abajo) para que esta medición ya sea la definitiva.
        ancho_pantalla = widget.width()
        ncols = max(1, int((ancho_pantalla - 60) / (max_width + 30)))

        # Alto real disponible: se mide el alto del propio canvas ya renderizado
        # (figure.bbox.height), NO la resolución de pantalla. Así, una ventana
        # pequeña en un monitor Full HD sigue calculando un número de filas
        # correcto para el espacio que realmente tiene disponible.
        alto_canvas_px = figure.bbox.height
        alto_maximo_leyenda = alto_canvas_px * FRACCION_MAX_ALTURA_LEYENDA
        alto_fila = max_height * 1.35 if max_height > 0 else 1  # interlineado/padding

        filas_maximas = int(alto_maximo_leyenda / alto_fila) if alto_fila > 0 else FILAS_LEYENDA_MIN
        filas_maximas = min(max(filas_maximas, FILAS_LEYENDA_MIN), FILAS_LEYENDA_MAX)

        items_por_pagina = ncols * filas_maximas

        return ncols, items_por_pagina, alto_maximo_leyenda

    def actualizar_leyenda(recalculado=False):
        try:
            ncols, items_por_pagina, alto_maximo_leyenda = calculate_columns()

            todos_handles = lineas + ([barras_pluviometro] if barras_pluviometro else [])
            todos_labels = [line.get_label() for line in lineas] + (["Precipitación"] if barras_pluviometro else [])
            total_items = len(todos_handles)

            # --- Paginación: partimos la leyenda en páginas de 'items_por_pagina' ---
            total_paginas = max(1, -(-total_items // items_por_pagina))  # techo (ceil)
            if estado_leyenda['pagina'] >= total_paginas:
                estado_leyenda['pagina'] = total_paginas - 1
            if estado_leyenda['pagina'] < 0:
                estado_leyenda['pagina'] = 0
            estado_leyenda['total_paginas'] = total_paginas

            inicio = estado_leyenda['pagina'] * items_por_pagina
            fin = inicio + items_por_pagina
            leyenda_elementos = todos_handles[inicio:fin]
            leyenda_labels = todos_labels[inicio:fin]

            requiere_paginacion = total_paginas > 1
            widget.boton_anterior_leyenda.setVisible(requiere_paginacion)
            widget.lbl_pagina_leyenda.setVisible(requiere_paginacion)
            widget.boton_siguiente_leyenda.setVisible(requiere_paginacion)
            if requiere_paginacion:
                widget.lbl_pagina_leyenda.setText(f"P\u00e1g. {estado_leyenda['pagina'] + 1}/{total_paginas}")
                widget.boton_anterior_leyenda.setEnabled(estado_leyenda['pagina'] > 0)
                widget.boton_siguiente_leyenda.setEnabled(estado_leyenda['pagina'] < total_paginas - 1)
            ...
            # Pasamos solo los elementos de la página visible; el resto de
            # series mantiene su estado (visible/oculta) aunque no se vean
            # en la leyenda en este momento.
            ...
            # --- AUTO-CORRECCIÓN: comparamos la altura REAL de la leyenda ya
            # renderizada contra el espacio máximo permitido. Si sobró espacio
            # (la estimación de calculate_columns fue conservadora) y todavía
            # quedan series fuera de esta página, recalculamos con una fila real
            # medida (no estimada) y volvemos a construir la leyenda UNA vez.
            # Esto la hace robusta ante fuentes/DPI que no midan exactamente
            # igual que nuestra estimación previa.
            if not recalculado and len(leyenda_elementos) > 0:
                filas_mostradas = -(-len(leyenda_elementos) // ncols)  # ceil
                alto_fila_real = legend_bbox.height / filas_mostradas
                filas_que_caben_real = int(alto_maximo_leyenda / alto_fila_real) if alto_fila_real > 0 else filas_mostradas
                items_por_pagina_real = max(ncols, ncols * filas_que_caben_real)
                # Solo recalculamos si el margen de mejora es significativo
                # (evita ciclos por diferencias mínimas de redondeo)
                if items_por_pagina_real >= items_por_pagina + ncols and total_items > items_por_pagina:
                    items_por_pagina = items_por_pagina_real
                    total_paginas = max(1, -(-total_items // items_por_pagina))
                    if estado_leyenda['pagina'] >= total_paginas:
                        estado_leyenda['pagina'] = total_paginas - 1
                    estado_leyenda['total_paginas'] = total_paginas
                    inicio = estado_leyenda['pagina'] * items_por_pagina
                    fin = inicio + items_por_pagina
                    leyenda_elementos_nuevo = todos_handles[inicio:fin]
                    leyenda_labels_nuevo = todos_labels[inicio:fin]
                    if len(leyenda_elementos_nuevo) > len(leyenda_elementos):
                        legend.remove()
                        requiere_paginacion = total_paginas > 1
                        widget.boton_anterior_leyenda.setVisible(requiere_paginacion)
                        widget.lbl_pagina_leyenda.setVisible(requiere_paginacion)
                        ...
                        if requiere_paginacion:
                            ...
                            widget.boton_anterior_leyenda.setEnabled(estado_leyenda['pagina'] > 0)
                            widget.boton_siguiente_leyenda.setEnabled(estado_leyenda['pagina'] < total_paginas - 1)
                        legend = ax.legend(handles=leyenda_elementos_nuevo, labels=leyenda_labels_nuevo,
                                        loc='upper center', bbox_to_anchor=(0.5, 0),
                                        ncol=ncols, frameon=False, fontsize=leyendazise,
                                        borderaxespad=0.8)
                        configurar_evento_leyenda(canvas, legend, leyenda_elementos_nuevo)
                        canvas.draw()
                        legend_bbox = legend.get_window_extent(renderer)
    ...
    def ir_pagina_anterior():
        if estado_leyenda['pagina'] > 0:
            estado_leyenda['pagina'] -= 1
            actualizar_leyenda()

    def ir_pagina_siguiente():
        if estado_leyenda['pagina'] < estado_leyenda['total_paginas'] - 1:
            estado_leyenda['pagina'] += 1
            actualizar_leyenda()

    widget.boton_anterior_leyenda.clicked.connect(ir_pagina_anterior)
    widget.boton_siguiente_leyenda.clicked.connect(ir_pagina_siguiente)

    canvas.mpl_connect('resize_event', on_resize)
    canvas.mpl_connect('button_press_event', on_click)
    canvas.mpl_connect('motion_notify_event', on_hover)
    # --- FIX posicionamiento inicial de la leyenda ---
    # Al momento de esta línea, Qt puede no haber terminado de acomodar
    # el layout (widget.width()/height() aún no reflejan el tamaño final),
    # por eso antes la leyenda se veía mal ubicada hasta que un evento
    # (como mover el mouse, que dispara un resize interno) la recalculaba.
    # QTimer.singleShot(0, ...) difiere la llamada al siguiente ciclo del
    # event loop de Qt, cuando el layout YA está resuelto, así se calcula
    # bien desde el primer render sin depender de que el usuario mueva algo.
    QTimer.singleShot(0, actualizar_leyenda)
    plt.close(figure)
    if avisolabels:
