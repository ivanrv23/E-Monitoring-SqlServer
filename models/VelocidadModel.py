...
class VelocidadModel:
    ...
    def mdlCalcularVelocidadVISD(tabla, unidad, prismas, idcomponente):
        ...
        # SQL Server: DATEDIFF para dias, FIRST_VALUE y LAG soportados (2012+).
        # Se asume que instrumentacion.id_componente es numérico o compatible.
        sql = f"""WITH velocidad AS (
            SELECT i.id_instrumentacion, p.nombre_prisma, p.hora_prisma,
                CAST(
                    DATEDIFF(SECOND, 
                        FIRST_VALUE(p.hora_prisma) OVER (PARTITION BY p.nombre_prisma ORDER BY p.nombre_prisma, p.hora_prisma),
                        p.hora_prisma
                    ) / 86400.0 AS FLOAT
                ) AS dias,
                CAST(
                    DATEDIFF(SECOND,
                        COALESCE(LAG(p.hora_prisma) OVER (PARTITION BY p.nombre_prisma ORDER BY p.nombre_prisma, p.hora_prisma), p.hora_prisma),
                        p.hora_prisma
                    ) / 86400.0 AS FLOAT
                ) AS dif_fechas,
    ...
    def mdlCalcularVelocidadDiasVISD(tabla, unidad, prismas, idcomponente, cantidad):
        ...
        # En SQL Server el GROUP BY debe incluir todas las columnas no agregadas.
        # DATE() se cambia por CAST(... AS DATE)
        ...
        bloques AS (
            SELECT i.id_instrumentacion, p.nombre_prisma, CAST(p.hora_prisma AS DATE) AS fecha, 
                ...
                FLOOR(DATEDIFF(DAY, f.fecha_inicio, CAST(p.hora_prisma AS DATE)) / CAST({cantidad} AS FLOAT)) AS bloque_dias,
                i.tipo_equipo
            FROM {tabla} p 
            INNER JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
            ...
            WHERE p.state_prisma = 1 AND p.estado_prisma = 1 
              AND p.nombre_prisma IN ({placeholders}) AND i.id_componente = ?
            GROUP BY p.nombre_prisma, i.id_instrumentacion, i.tipo_equipo, f.fecha_inicio, CAST(p.hora_prisma AS DATE), 
                     FLOOR(DATEDIFF(DAY, f.fecha_inicio, CAST(p.hora_prisma AS DATE)) / CAST({cantidad} AS FLOAT))
        ),
        velocidad AS (
            SELECT b.id_instrumentacion, b.nombre_prisma, b.hora_prisma,
                CAST(
                    DATEDIFF(SECOND,
                        FIRST_VALUE(b.hora_prisma) OVER (PARTITION BY b.nombre_prisma ORDER BY b.nombre_prisma, b.hora_prisma),
                        b.hora_prisma
                    ) / 86400.0 AS FLOAT
                ) AS dias,
                CAST(
                    DATEDIFF(SECOND,
                        COALESCE(LAG(b.hora_prisma) OVER (PARTITION BY b.nombre_prisma ORDER BY b.nombre_prisma, b.hora_prisma), b.hora_prisma),
                        b.hora_prisma
                    ) / 86400.0 AS FLOAT
                ) AS dif_fechas,
    ...
    def mdlCalcularVelocidadHorasVISD(tabla, unidad, prismas, idcomponente, cantidad):
        ...
        sql = f"""WITH promedios_horas AS (
            ...
            WHERE p.state_prisma = 1 AND p.estado_prisma = 1 
              AND p.nombre_prisma IN ({placeholders}) AND i.id_componente = ?
            GROUP BY p.nombre_prisma, i.id_instrumentacion, i.tipo_equipo, CAST(p.hora_prisma AS DATE), DATEPART(HOUR, p.hora_prisma) / {cantidad}
        ),
        velocidad AS (
            SELECT pd.id_instrumentacion, pd.nombre_prisma, pd.hora_prisma,
                CAST(
                    DATEDIFF(SECOND,
                        FIRST_VALUE(pd.hora_prisma) OVER (PARTITION BY pd.nombre_prisma ORDER BY pd.nombre_prisma, pd.hora_prisma),
                        pd.hora_prisma
                    ) / 86400.0 AS FLOAT
                ) AS dias,
                CAST(
                    DATEDIFF(SECOND,
                        COALESCE(LAG(pd.hora_prisma) OVER (PARTITION BY pd.nombre_prisma ORDER BY pd.nombre_prisma, pd.hora_prisma), pd.hora_prisma),
                        pd.hora_prisma
                    ) / 86400.0 AS FLOAT
                ) AS dif_fechas,
    ...
    def mdlCalcularVelocidadHorasVASD(tabla, unidad, prismas, idcomponente, cantidad):
        ...
        # Cambio: STRFTIME -> DATEPART, GROUP BY explícito, DATEDIFF
        sql = f"""WITH promedios_horas AS (
            SELECT i.id_instrumentacion, p.nombre_prisma, CAST(p.hora_prisma AS DATE) AS fecha,
                DATEPART(HOUR, p.hora_prisma) / {cantidad} AS bloque,
                MAX(p.hora_prisma) AS hora_prisma, 
    ...
    def mdlCalcularVelocidadVI2D(tabla, unidad, prismas, idcomponente):
        ...
        # Nota: ROW_NUMBER y LAG son compatibles. Julianday se cambia por DATEDIFF.
        # En la division final, se usa DATEDIFF entre la hora actual y la anterior (LAG).
        ...
        SELECT id_instrumentacion, nombre_prisma, hora_prisma AS FECHAS, dias AS DIAS, dias * 24 AS HORAS,
            CASE
                ...
                / (NULLIF(DATEDIFF(SECOND, 
                        LAG(hora_prisma, 1, hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY nombre_prisma, hora_prisma), 
                        hora_prisma
                   ), 0) / 86400.0)
    ...
    def mdlCalcularVelocidadVA2D(tabla, unidad, prismas, idcomponente):
        ...
        # Mantenemos el alias VA3D al final si así lo requería el frontend original, 
        # aunque el cálculo es sobre 2D (VA2D).
        sql = f"""WITH PrismasCTE AS (
            SELECT i.id_instrumentacion, p.nombre_prisma, p.hora_prisma, p.este_target, p.norte_target,
                CAST(
                    DATEDIFF(SECOND,
        ...
        SELECT id_instrumentacion, nombre_prisma, hora_prisma AS FECHAS, dias AS DIAS, dias * 24 AS HORAS,
            CASE 
                WHEN DATEDIFF(SECOND, FIRST_VALUE(hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY nombre_prisma, hora_prisma), hora_prisma) = 0 THEN 0
                ELSE (dosD - FIRST_VALUE(dosD) OVER (PARTITION BY nombre_prisma ORDER BY nombre_prisma, hora_prisma))
                    / (DATEDIFF(SECOND, 
                            FIRST_VALUE(hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY nombre_prisma, hora_prisma), 
                            hora_prisma
                       ) / 86400.0)
    ...
    def mdlCalcularVelocidadPositivaVI3D(tabla, unidad, prismas, idcomponente):
        ...
        # SQL Server: DATEDIFF / 86400.0, NULLIF para evitar division por cero
        sql = f"""WITH PrismasCTE AS (
        ...
        SELECT id_instrumentacion, nombre_prisma, hora_prisma AS FECHAS, dias AS DIAS, dias * 24 AS HORAS,
        CASE 
            WHEN LAG(hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY nombre_prisma, hora_prisma) IS NULL THEN 0
            ELSE tresD / (NULLIF(DATEDIFF(SECOND, 
                                    LAG(hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY nombre_prisma, hora_prisma), 
                                    hora_prisma
                                 ), 0) / 86400.0)
