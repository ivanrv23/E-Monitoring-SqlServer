...
class VelocidadModel:
    ...
    def mdlCalcularVelocidadVISD(tabla, unidad, prismas, idcomponente, fechaini, fechafin):
        ...
        try:
            ...
            # AVISO: VI (Velocidad Incremental) usa LAG(). No necesita la CTE ValoresCero, 
            # solo se debe aplicar el filtro de fecha para que LAG() opere en un conjunto reducido.
            sql = f"""WITH datos_filtrados AS (
                ...
                AND i.id_componente = ? AND p.nombre_prisma IN ({placeholders})
                AND p.hora_prisma <= ? -- Incluir un dato previo para el cálculo de LAG
                UNION
                SELECT
                    i.id_instrumentacion,
                    p.nombre_prisma,
            ...
            velocidad AS (
                SELECT
                    id_instrumentacion, nombre_prisma, hora_prisma, tipo_equipo,
            ...
            WHERE cf.hora_prisma BETWEEN ? AND ?
    ...
    def mdlCalcularVelocidadDiasVISD(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            placeholders = ', '.join(['?' for _ in prismas])
            sql = f"""
            WITH datos_para_promedio AS (
                -- Bloque de datos en el rango solicitado
                ...
                UNION ALL
                -- El último día de lectura ANTES del rango (para el cálculo de LAG)
                SELECT p.nombre_prisma, p.hora_prisma, p.distancia_prisma, i.id_instrumentacion, i.tipo_equipo
                FROM {tabla} p JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ?
                  AND CAST(p.hora_prisma AS DATE) = (
                    SELECT MAX(CAST(p_inner.hora_prisma AS DATE))
                    FROM {tabla} p_inner
                    WHERE p_inner.nombre_prisma = p.nombre_prisma AND p_inner.hora_prisma < ?
                  )
            ),
            bloques AS (
                SELECT
                    nombre_prisma,
                    CAST(hora_prisma AS DATE) as fecha_bloque,
                    MAX(hora_prisma) AS hora_prisma,
                    AVG(CAST(distancia_prisma AS FLOAT)) AS promedio_distancia,
                    MAX(id_instrumentacion) AS id_instrumentacion,
                    MAX(tipo_equipo) AS tipo_equipo
                FROM datos_para_promedio
                GROUP BY nombre_prisma, CAST(hora_prisma AS DATE)
            ),
            velocidad AS (
                SELECT *,
                    CAST(DATEDIFF(SECOND, COALESCE(LAG(hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), hora_prisma), hora_prisma) AS FLOAT) / 86400.0 AS dif_fechas,
            ...
            SELECT
                id_instrumentacion, nombre_prisma, hora_prisma AS FECHAS, 0 AS DIAS, 0 AS HORAS,
                CASE WHEN dif_fechas = 0 THEN 0 ELSE dif_sd / dif_fechas END AS VISD,
                tipo_equipo
            FROM velocidad
            WHERE hora_prisma BETWEEN ? AND ?
            ORDER BY nombre_prisma, hora_prisma;
            """
    ...
    def mdlCalcularVelocidadHorasVISD(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            WITH datos_para_promedio AS (
                ...
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ? AND p.hora_prisma BETWEEN ? AND ?
                UNION ALL
                -- Último bloque de horas ANTES del rango (para LAG)
                SELECT p.nombre_prisma, p.hora_prisma, p.distancia_prisma, i.id_instrumentacion, i.tipo_equipo
                FROM {tabla} p JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ?
                  AND p.hora_prisma IN (
                    SELECT TOP 1 WITH TIES p_inner.hora_prisma
                    FROM {tabla} p_inner
                    WHERE p_inner.nombre_prisma = p.nombre_prisma AND p_inner.hora_prisma < ?
                    ORDER BY p_inner.hora_prisma DESC
                  )
            ),
            bloques AS (
                SELECT
                    ...
                    MAX(tipo_equipo) AS tipo_equipo
                FROM datos_para_promedio
                GROUP BY nombre_prisma, CAST(hora_prisma AS DATE), DATEPART(HOUR, hora_prisma) / {cantidad}
            ),
            velocidad AS (
                SELECT *,
                    CAST(DATEDIFF(SECOND, COALESCE(LAG(hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), hora_prisma), hora_prisma) AS FLOAT) / 86400.0 AS dif_fechas,
    ...
    def mdlCalcularVelocidadVASD(tabla, unidad, prismas, idcomponente, fechaini, fechafin):
        ...
        try:
            placeholders = ', '.join(['?' for _ in prismas])
            # AVISO: VA (Velocidad Acumulada) usa FIRST_VALUE(). Necesita la CTE ValoresCero 
            # para encontrar el "cero absoluto" de forma eficiente.
            sql = f"""
            WITH ValoresCero AS (
                SELECT 
                    p_cte.nombre_prisma,
                ...
                FROM (
                    ...
                    FROM {tabla}
                    WHERE nombre_prisma IN ({placeholders}) AND state_prisma = 1 AND estado_prisma = 1
                ) p_cte
                WHERE p_cte.rn = 1
            )
            SELECT 
                ...
                CASE
                    WHEN (CAST(DATEDIFF(SECOND, vc.hora_cero, datos.hora_prisma) AS FLOAT) / 86400.0) = 0 THEN 0
                    ELSE ((datos.distancia_prisma - vc.valor_cero) * ?) / (CAST(DATEDIFF(SECOND, vc.hora_cero, datos.hora_prisma) AS FLOAT) / 86400.0)
                END AS VASD,
                i.tipo_equipo
            FROM {tabla} datos
    ...
    def mdlCalcularVelocidadDiasVASD(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            sql = f"""
            WITH PrimerDia AS (
                SELECT nombre_prisma, MIN(CAST(hora_prisma AS DATE)) as fecha_cero
                FROM {tabla}
                WHERE nombre_prisma IN ({placeholders}) AND state_prisma = 1 AND estado_prisma = 1
            ...
            ValoresCero AS (
                SELECT
                    p.nombre_prisma,
                    MAX(p.hora_prisma) as hora_cero,
                    AVG(CAST(p.distancia_prisma AS FLOAT)) as valor_cero
                FROM {tabla} p
                JOIN PrimerDia pd ON p.nombre_prisma = pd.nombre_prisma AND CAST(p.hora_prisma AS DATE) = pd.fecha_cero
                WHERE p.state_prisma = 1 AND p.estado_prisma = 1
                GROUP BY p.nombre_prisma, pd.fecha_cero
            ),
            BloquesEnRango AS (
                SELECT
                    p.nombre_prisma,
                    MAX(p.hora_prisma) as hora_bloque,
                    AVG(CAST(p.distancia_prisma AS FLOAT)) as valor_bloque,
                    MAX(i.id_instrumentacion) as id_instrumentacion,
                    MAX(i.tipo_equipo) as tipo_equipo
                FROM {tabla} p
                JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ?
            ...
            SELECT
                br.id_instrumentacion,
                br.nombre_prisma,
                br.hora_bloque AS FECHAS,
                ...
                CASE
                    WHEN (CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0) = 0 THEN 0
                    ELSE ((br.valor_bloque - vc.valor_cero) * ?) / (CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0)
                END AS VASD,
                br.tipo_equipo
            FROM BloquesEnRango br
            JOIN ValoresCero vc ON br.nombre_prisma = vc.nombre_prisma
            ORDER BY br.nombre_prisma, br.hora_bloque;
            """
    ...
    def mdlCalcularVelocidadHorasVASD(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            placeholders = ', '.join(['?' for _ in prismas])
            sql = f"""
            WITH PrimerBloque AS (
                SELECT nombre_prisma, MIN(hora_prisma) as hora_cero_aprox
                FROM {tabla}
                WHERE nombre_prisma IN ({placeholders}) AND state_prisma = 1 AND estado_prisma = 1
                GROUP BY nombre_prisma
            ),
            ValoresCero AS (
                SELECT
                    ...
                    AVG(CAST(p.distancia_prisma AS FLOAT)) as valor_cero
                FROM {tabla} p
                JOIN PrimerBloque pb ON p.nombre_prisma = pb.nombre_prisma
                WHERE p.state_prisma = 1 AND p.estado_prisma = 1
                  AND CAST(p.hora_prisma AS DATE) = CAST(pb.hora_cero_aprox AS DATE)
                  AND DATEPART(HOUR, p.hora_prisma) / {cantidad} = DATEPART(HOUR, pb.hora_cero_aprox) / {cantidad}
                GROUP BY p.nombre_prisma
            ),
            BloquesEnRango AS (
                SELECT
            ...
            SELECT
                br.id_instrumentacion, br.nombre_prisma, br.hora_bloque AS FECHAS,
                CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0 AS DIAS,
                (CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0) * 24.0 AS HORAS,
                CASE
                    WHEN (CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0) = 0 THEN 0
        ...
        try:
            ...
            WITH datos_filtrados AS (
                SELECT 
                    ...
                    p.este_target, p.norte_target, i.tipo_equipo
                FROM (
                    SELECT *, ROW_NUMBER() OVER(PARTITION BY nombre_prisma ORDER BY hora_prisma DESC) as rn
                    FROM {tabla}
                    WHERE nombre_prisma IN ({placeholders}) AND hora_prisma < ?
                ) p
                INNER JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.rn = 1 AND i.id_componente = ?
                UNION ALL
                SELECT 
                    i.id_instrumentacion, p.nombre_prisma, p.hora_prisma,
                    p.este_target, p.norte_target, i.tipo_equipo
            ...
            SELECT 
                id_instrumentacion, nombre_prisma, hora_prisma AS FECHAS, dias AS DIAS, dias * 24.0 AS HORAS,
                CASE
                    WHEN dif_dias = 0 THEN 0
                    ELSE (dosD - LAG(dosD) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma)) / dif_dias
                END AS VI2D, 
                tipo_equipo
            FROM PrismasCTE
            WHERE hora_prisma BETWEEN ? AND ?
    ...
    def mdlCalcularVelocidadDiasVI2D(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            WITH datos_para_promedio AS (
                -- Datos en el rango
                SELECT p.nombre_prisma, p.hora_prisma, p.este_target, p.norte_target, i.id_instrumentacion, i.tipo_equipo
                FROM {tabla} p JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ? AND p.hora_prisma BETWEEN ? AND ?
                UNION ALL
                -- Último día de lectura ANTES del rango
                SELECT p.nombre_prisma, p.hora_prisma, p.este_target, p.norte_target, i.id_instrumentacion, i.tipo_equipo
                FROM {tabla} p JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ?
                  AND CAST(p.hora_prisma AS DATE) = (
            ...
            bloques AS (
                SELECT
                    nombre_prisma,
                    MAX(hora_prisma) AS hora_prisma,
                    AVG(CAST(este_target AS FLOAT)) AS promedio_este,
                    AVG(CAST(norte_target AS FLOAT)) AS promedio_norte,
                    MAX(id_instrumentacion) AS id_instrumentacion,
                    MAX(tipo_equipo) AS tipo_equipo
                FROM datos_para_promedio
                GROUP BY nombre_prisma, CAST(hora_prisma AS DATE)
            ...
            velocidad AS (
                SELECT *,
                   SQRT(
                        POWER(promedio_este - LAG(promedio_este) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), 2) +
                        POWER(promedio_norte - LAG(promedio_norte) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), 2)
                   ) * ? AS dosD,
            ...
            SELECT
                id_instrumentacion, nombre_prisma, hora_prisma AS FECHAS, 0 AS DIAS, 0 AS HORAS,
                CASE WHEN dif_dias = 0 THEN 0 ELSE (dosD - LAG(dosD) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma)) / dif_dias END AS VI2D,
                tipo_equipo
            FROM velocidad
            WHERE hora_prisma BETWEEN ? AND ?
            ORDER BY nombre_prisma, hora_prisma;
    ...
    def mdlCalcularVelocidadHorasVI2D(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            WITH datos_para_promedio AS (
                ...
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ? AND p.hora_prisma BETWEEN ? AND ?
                UNION ALL
                -- Último bloque de horas ANTES del rango
                SELECT p.nombre_prisma, p.hora_prisma, p.este_target, p.norte_target, i.id_instrumentacion, i.tipo_equipo
                FROM {tabla} p JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ?
                  AND p.hora_prisma IN (
            ...
            bloques AS (
                SELECT
                    nombre_prisma, MAX(hora_prisma) AS hora_prisma,
                    AVG(CAST(este_target AS FLOAT)) AS promedio_este,
                    AVG(CAST(norte_target AS FLOAT)) AS promedio_norte,
                    MAX(id_instrumentacion) AS id_instrumentacion, MAX(tipo_equipo) AS tipo_equipo
                FROM datos_para_promedio
                GROUP BY nombre_prisma, CAST(hora_prisma AS DATE), DATEPART(HOUR, hora_prisma) / {cantidad}
            ),
            velocidad AS (
    ...
    def mdlCalcularVelocidadVA2D(tabla, unidad, prismas, idcomponente, fechaini, fechafin):
        ...
        try:
            ...
            WITH ValoresCero AS (
                SELECT 
                    p_cte.nombre_prisma,
                    p_cte.este_target AS este_cero,
                    p_cte.norte_target AS norte_cero,
                    p_cte.hora_prisma AS hora_cero
                FROM (
                    SELECT
            ...
            CalculoCTE AS (
                SELECT 
                    i.id_instrumentacion, datos.nombre_prisma, datos.hora_prisma, i.tipo_equipo,
                    (CAST(DATEDIFF(SECOND, vc.hora_cero, datos.hora_prisma) AS FLOAT) / 86400.0) AS dias,
                    SQRT(
                        POWER(datos.este_target - vc.este_cero, 2) +
                        POWER(datos.norte_target - vc.norte_cero, 2)
                    ) * ? AS dosD
                FROM {tabla} datos
                INNER JOIN instrumentacion i ON datos.nombre_prisma = i.nombre_equipo
                INNER JOIN ValoresCero vc ON datos.nombre_prisma = vc.nombre_prisma
        ...
        try:
            ...
            ValoresCero AS (
                SELECT
                    p.nombre_prisma,
                    MAX(p.hora_prisma) as hora_cero,
                    AVG(CAST(p.este_target AS FLOAT)) as este_cero,
                    AVG(CAST(p.norte_target AS FLOAT)) as norte_cero
                FROM {tabla} p
                JOIN PrimerDia pd ON p.nombre_prisma = pd.nombre_prisma AND CAST(p.hora_prisma AS DATE) = pd.fecha_cero
                WHERE p.state_prisma = 1 AND p.estado_prisma = 1
                GROUP BY p.nombre_prisma, pd.fecha_cero
            ...
            BloquesEnRango AS (
                SELECT
                    p.nombre_prisma,
                    MAX(p.hora_prisma) as hora_bloque,
                    AVG(CAST(p.este_target AS FLOAT)) as este_bloque,
                    AVG(CAST(p.norte_target AS FLOAT)) as norte_bloque,
                    MAX(i.id_instrumentacion) as id_instrumentacion,
                    MAX(i.tipo_equipo) as tipo_equipo
                FROM {tabla} p
                JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
            ...
            SELECT
                ...
                CASE
                    WHEN (CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0) = 0 THEN 0
                    ELSE (SQRT(POWER(br.este_bloque - vc.este_cero, 2) + POWER(br.norte_bloque - vc.norte_cero, 2)) * ?) / (CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0)
                END AS VA2D, -- ALIAS CORREGIDO
                br.tipo_equipo
            FROM BloquesEnRango br
            JOIN ValoresCero vc ON br.nombre_prisma = vc.nombre_prisma
    ...
    def mdlCalcularVelocidadHorasVA2D(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            WITH PrimerBloque AS (
                SELECT nombre_prisma, MIN(hora_prisma) as hora_cero_aprox
                FROM {tabla} WHERE nombre_prisma IN ({placeholders}) AND state_prisma = 1 AND estado_prisma = 1 GROUP BY nombre_prisma
            ),
            ValoresCero AS (
                SELECT p.nombre_prisma, MAX(p.hora_prisma) as hora_cero,
                    AVG(CAST(p.este_target AS FLOAT)) as este_cero, AVG(CAST(p.norte_target AS FLOAT)) as norte_cero
            ...
            BloquesEnRango AS (
                SELECT p.nombre_prisma, MAX(p.hora_prisma) as hora_bloque,
                    AVG(CAST(p.este_target AS FLOAT)) as este_bloque, AVG(CAST(p.norte_target AS FLOAT)) as norte_bloque,
                    MAX(i.id_instrumentacion) as id_instrumentacion, MAX(i.tipo_equipo) as tipo_equipo
                FROM {tabla} p JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
    ...
    def mdlCalcularVelocidadPositivaVI3D(tabla, unidad, prismas, idcomponente, fechaini, fechafin):
        ...
        try:
            ...
            WITH datos_filtrados AS (
                SELECT 
                    i.id_instrumentacion, p.nombre_prisma, p.hora_prisma,
                    p.este_target, p.norte_target, p.elevacion_target, i.tipo_equipo
                FROM (
                    SELECT *, ROW_NUMBER() OVER(PARTITION BY nombre_prisma ORDER BY hora_prisma DESC) as rn
                    FROM {tabla}
                    WHERE nombre_prisma IN ({placeholders}) AND hora_prisma < ?
    ...
    def mdlCalcularVelocidadDiasPositivaVI3D(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            sql = f"""
            WITH datos_para_promedio AS (
                SELECT p.nombre_prisma, p.hora_prisma, p.este_target, p.norte_target, p.elevacion_target, i.id_instrumentacion, i.tipo_equipo
                FROM {tabla} p JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ? AND p.hora_prisma BETWEEN ? AND ?
                UNION ALL
                SELECT p.nombre_prisma, p.hora_prisma, p.este_target, p.norte_target, p.elevacion_target, i.id_instrumentacion, i.tipo_equipo
            ...
            bloques AS (
                SELECT
                    ...
                    AVG(CAST(este_target AS FLOAT)) AS promedio_este,
                    AVG(CAST(norte_target AS FLOAT)) AS promedio_norte,
                    AVG(CAST(elevacion_target AS FLOAT)) AS promedio_elevacion,
                    MAX(id_instrumentacion) AS id_instrumentacion,
                    MAX(tipo_equipo) AS tipo_equipo
                FROM datos_para_promedio
                GROUP BY nombre_prisma, CAST(hora_prisma AS DATE)
            ...
            velocidad AS (
                SELECT *,
                   SQRT(
                        POWER(promedio_este - LAG(promedio_este) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), 2) +
                        POWER(promedio_norte - LAG(promedio_norte) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), 2) +
                        POWER(promedio_elevacion - LAG(promedio_elevacion) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), 2)
                   ) * ? AS tresD,
                   CAST(DATEDIFF(SECOND, LAG(hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), hora_prisma) AS FLOAT) / 86400.0 AS dif_dias
                FROM bloques
            )
            SELECT
                id_instrumentacion, nombre_prisma, hora_prisma AS FECHAS, 0 AS DIAS, 0 AS HORAS,
                CASE WHEN dif_dias = 0 THEN 0 ELSE tresD / dif_dias END AS VI3D,
                tipo_equipo
            FROM velocidad
            WHERE hora_prisma BETWEEN ? AND ?
            ORDER BY nombre_prisma, hora_prisma;
    ...
    def mdlCalcularVelocidadHorasPositivaVI3D(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            bloques AS (
                SELECT
                    nombre_prisma, MAX(hora_prisma) AS hora_prisma,
                    AVG(CAST(este_target AS FLOAT)) AS promedio_este, AVG(CAST(norte_target AS FLOAT)) AS promedio_norte, AVG(CAST(elevacion_target AS FLOAT)) AS promedio_elevacion,
                    MAX(id_instrumentacion) AS id_instrumentacion, MAX(tipo_equipo) AS tipo_equipo
                FROM datos_para_promedio
                GROUP BY nombre_prisma, CAST(hora_prisma AS DATE), DATEPART(HOUR, hora_prisma) / {cantidad}
            ),
            velocidad AS (
                SELECT *,
                   SQRT(POWER(promedio_este - LAG(promedio_este) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), 2) + POWER(promedio_norte - LAG(promedio_norte) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), 2) + POWER(promedio_elevacion - LAG(promedio_elevacion) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), 2)) * ? AS tresD,
                   CAST(DATEDIFF(SECOND, LAG(hora_prisma) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma), hora_prisma) AS FLOAT) / 86400.0 AS dif_dias
                FROM bloques
            )
            SELECT
    ...
    def mdlCalcularVelocidadVI3D(tabla, unidad, prismas, idcomponente, fechaini, fechafin):
        ...
        try:
            ...
            SELECT 
                ...
                CASE
                    WHEN dif_dias = 0 THEN 0
                    ELSE (tresD - LAG(tresD) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma)) / dif_dias
                END AS VI3D, 
                tipo_equipo
            FROM PrismasCTE
            WHERE hora_prisma BETWEEN ? AND ?
    ...
    def mdlCalcularVelocidadDiasVI3D(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            SELECT
                id_instrumentacion, nombre_prisma, hora_prisma AS FECHAS, 0 AS DIAS, 0 AS HORAS,
                CASE WHEN dif_dias = 0 THEN 0 ELSE (tresD - LAG(tresD) OVER (PARTITION BY nombre_prisma ORDER BY hora_prisma)) / dif_dias END AS VI3D,
                tipo_equipo
            FROM velocidad
            WHERE hora_prisma BETWEEN ? AND ?
            ORDER BY nombre_prisma, hora_prisma;
        ...
        try:
            ...
            WITH ValoresCero AS (
                SELECT 
                    ...
                    p_cte.este_target AS este_cero,
                    p_cte.norte_target AS norte_cero,
                    p_cte.elevacion_target AS elevacion_cero,
                    p_cte.hora_prisma AS hora_cero
                FROM (
                    SELECT
            ...
            CalculoCTE AS (
                SELECT 
                    ...
                    SQRT(
                        POWER(datos.este_target - vc.este_cero, 2) +
                        POWER(datos.norte_target - vc.norte_cero, 2) +
                        POWER(datos.elevacion_target - vc.elevacion_cero, 2)
                    ) * ? AS tresD
                FROM {tabla} datos
                INNER JOIN instrumentacion i ON datos.nombre_prisma = i.nombre_equipo
                INNER JOIN ValoresCero vc ON datos.nombre_prisma = vc.nombre_prisma
        ...
        try:
            ...
            ValoresCero AS (
                SELECT
                    ...
                    AVG(CAST(p.este_target AS FLOAT)) as este_cero,
                    AVG(CAST(p.norte_target AS FLOAT)) as norte_cero,
                    AVG(CAST(p.elevacion_target AS FLOAT)) as elevacion_cero
                FROM {tabla} p
                JOIN PrimerDia pd ON p.nombre_prisma = pd.nombre_prisma AND CAST(p.hora_prisma AS DATE) = pd.fecha_cero
                WHERE p.state_prisma = 1 AND p.estado_prisma = 1
                GROUP BY p.nombre_prisma, pd.fecha_cero
            ...
            BloquesEnRango AS (
                SELECT
                    ...
                    AVG(CAST(p.este_target AS FLOAT)) as este_bloque,
                    AVG(CAST(p.norte_target AS FLOAT)) as norte_bloque,
                    AVG(CAST(p.elevacion_target AS FLOAT)) as elevacion_bloque,
                    MAX(i.id_instrumentacion) as id_instrumentacion,
                    MAX(i.tipo_equipo) as tipo_equipo
                FROM {tabla} p
                JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
            ...
            SELECT
                ...
                CASE
                    WHEN (CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0) = 0 THEN 0
                    ELSE (SQRT(POWER(br.este_bloque - vc.este_cero, 2) + POWER(br.norte_bloque - vc.norte_cero, 2) + POWER(br.elevacion_bloque - vc.elevacion_cero, 2)) * ?) / (CAST(DATEDIFF(SECOND, vc.hora_cero, br.hora_bloque) AS FLOAT) / 86400.0)
                END AS VA3D,
                br.tipo_equipo
            FROM BloquesEnRango br
            JOIN ValoresCero vc ON br.nombre_prisma = vc.nombre_prisma
    ...
    def mdlCalcularVelocidadHorasVA3D(tabla, unidad, prismas, idcomponente, fechaini, fechafin, cantidad):
        ...
        try:
            ...
            ValoresCero AS (
                SELECT p.nombre_prisma, MAX(p.hora_prisma) as hora_cero,
                    AVG(CAST(p.este_target AS FLOAT)) as este_cero, AVG(CAST(p.norte_target AS FLOAT)) as norte_cero, AVG(CAST(p.elevacion_target AS FLOAT)) as elevacion_cero
                FROM {tabla} p JOIN PrimerBloque pb ON p.nombre_prisma = pb.nombre_prisma
                WHERE p.state_prisma = 1 AND p.estado_prisma = 1
                  AND CAST(p.hora_prisma AS DATE) = CAST(pb.hora_cero_aprox AS DATE)
                  AND DATEPART(HOUR, p.hora_prisma) / {cantidad} = DATEPART(HOUR, pb.hora_cero_aprox) / {cantidad}
            ...
            BloquesEnRango AS (
                SELECT p.nombre_prisma, MAX(p.hora_prisma) as hora_bloque,
                    AVG(CAST(p.este_target AS FLOAT)) as este_bloque, AVG(CAST(p.norte_target AS FLOAT)) as norte_bloque, AVG(CAST(p.elevacion_target AS FLOAT)) as elevacion_bloque,
                    MAX(i.id_instrumentacion) as id_instrumentacion, MAX(i.tipo_equipo) as tipo_equipo
                FROM {tabla} p JOIN instrumentacion i ON p.nombre_prisma = i.nombre_equipo
                WHERE p.nombre_prisma IN ({placeholders}) AND i.id_componente = ? AND p.hora_prisma BETWEEN ? AND ? AND p.state_prisma = 1 AND p.estado_prisma = 1
                GROUP BY p.nombre_prisma, CAST(p.hora_prisma AS DATE), DATEPART(HOUR, p.hora_prisma) / {cantidad}
