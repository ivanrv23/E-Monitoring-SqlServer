...
class CeldaModel:
    ...
    def mdlCalcularVelocidadDias(dias, tabla, idcomponente, listaceldas):
        ...
        sql = f"""WITH IncrementalCTE AS (
            SELECT t.id_instrumentacion, c.nombre_celda, cd.fecha_detalle,
                CAST(julianday(cd.fecha_detalle) - julianday(FIRST_VALUE(cd.fecha_detalle) OVER (PARTITION BY c.nombre_celda ORDER BY cd.fecha_detalle)) AS NUMERIC) AS dias,
                CAST(julianday(cd.fecha_detalle) - julianday(FIRST_VALUE(cd.fecha_detalle) OVER (PARTITION BY c.nombre_celda ORDER BY cd.fecha_detalle)) AS NUMERIC) * 24 AS horas,
                cd.medida_calculada, c.fundacion_celda,
                COALESCE(
                    (SELECT c2.nivel_cota FROM cotas_celdas c2 WHERE c2.id_celda = c.id_celda 
                    AND c2.fecha_cota <= cd.fecha_detalle ORDER BY c2.fecha_cota DESC LIMIT 1),
                    (SELECT c3.nivel_cota FROM cotas_celdas c3 WHERE c3.id_celda = c.id_celda 
        ...
        GroupedSummary AS (
            ...
            CAST((julianday(fecha_detalle) - julianday((SELECT MIN(fecha_detalle) FROM {tabla}))) / ? AS INTEGER) AS grupo_dias,
        ...
        AggregatedSummary AS (
            ...
            FROM GroupedSummary GROUP BY nombre_celda, grupo_dias
        )
    ...
    def mdlCalcularVelocidadMes(tabla, idcomponente, listaceldas):
        ...
        MonthlySummary AS (
            SELECT id_instrumentacion, nombre_celda, strftime('%Y-%m', fecha_detalle) AS mes, fecha_detalle, dias, horas, incremental,
                SUM(incremental) OVER (PARTITION BY nombre_celda, strftime('%Y-%m', fecha_detalle)) AS velocidad_metros,
                fundacion_celda, superficie,
                ROW_NUMBER() OVER (PARTITION BY nombre_celda, strftime('%Y-%m', fecha_detalle) ORDER BY fecha_detalle DESC) AS rn,
    ...
    def mdlRegistrarDataCelda(proyectoid, data, idsceldas):
        ...
        try:
            ...
            existen_celdas = set([(row[0], row[1]) for row in cursor.execute(f"SELECT id_celda, fecha_detalle FROM {table_name} WHERE id_celda IN ({placeholders});", list(idsceldas))])
            lote_registros = []
            contador = 0
            for fila in data:
                id_celda = fila[0]
